...
export const IngredientSearchInput = ({ onSelect, placeholder = "Search for ingredients..." }: IngredientSearchInputProps) => {
  ...
  // Search ingredients from Supabase using optimized search function
  const searchIngredients = async (searchQuery: string) => {
    ...
    try {
      ...
      if (error) {
        logger.error('Error searching ingredients (optimized):', error);
        ...
        if (fallbackError) {
          logger.error('Error searching ingredients (fallback):', fallbackError);
          setSuggestions([]);
          setIsOpen(false);
        } else {
          const ingredients = (fallbackData || []) as unknown as IngredientSuggestion[];
